...
import ontInterface.USelectionsOwl
...
object SparkNaiveBayes {
  ...
  def main(args: Array[String]) {
    ...
    val CATEGORIES = List("CandidateElection", "PresidentElection")
    var index = 0
    ...
    resultData.map(f => {
      ...
      var className = ""
      if (f._2 == 0)
        className = ":CandidateElection"
      else
        className = ":PresidentElection"
